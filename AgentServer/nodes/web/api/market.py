...
from .system_sync import get_market_core_readiness_marker
...
@router.get("/core-readiness")
async def get_market_core_readiness(
    trade_date: Optional[str] = Query(None, description="交易日，格式 YYYYMMDD；不传则返回最新标记"),
) -> Dict[str, Any]:
    """获取核心市场数据可用性标记，供市场页决定数据是否可直接使用。"""
    normalized_trade_date = _normalize_trade_date_input(trade_date)
    return await get_market_core_readiness_marker(trade_date=normalized_trade_date)
