...
class MilvusManager(BaseManager):
    ...
    async def initialize(self) -> None:
        ...
        self._config.host = normalize_local_service_host(self._config.host, "milvus")

        uri = self._build_uri()
        restore_env_uri = None

        # pymilvus 在 import 时会读取 MILVUS_URI 环境变量并校验为远程 URI。
        # 独立 DataSync 默认使用本地 Milvus Lite 文件路径，因此需要避免把
        # 本地路径直接暴露给 pymilvus 的全局配置解析。
        if self._is_local_path(uri):
            restore_env_uri = os.environ.pop("MILVUS_URI", None)

        try:
            from pymilvus import MilvusClient
        finally:
            if restore_env_uri is not None:
                os.environ["MILVUS_URI"] = restore_env_uri
    ...
    async def _init_lite_mode(self, MilvusClient, uri: str) -> None:
        ...
        try:
            ...
            message = str(e).lower()
            if "milvus-lite" in message or "milvus_lite" in message:
                self._is_disabled = True
                self._client = None
                self.logger.warning(
                    "Milvus Lite dependency unavailable. "
                    "Vector search features are DISABLED. "
                    "Install DataSync requirements with the milvus lite extra to enable it."
                )
                return
            raise
