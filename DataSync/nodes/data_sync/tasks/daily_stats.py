...
from core.managers.analysis_manager import analysis_manager
...
class DailyStatsTask(BaseTask):
    ...
    - 默认: 每个交易日 16:30 (在收盘后核心行情链路完成后尽快产出)
    """
    ...
    async def recover_trade_date(self, trade_date: str) -> Dict[str, Any]:
        ...
        result = await self._compute_single_day(trade_date, force=True)
        return {
            "success": True,
            "count": 1,
            "trade_date": trade_date,
            "source": "local_compute",
            "sources": ["local_compute"],
            "warnings": build_recovery_warnings(guard),
            **result,
            "message": f"Recovered daily_stats for {trade_date}",
        }
