...
class HotNewsSource(ABC):
    ...
    def __init__(self):
        self.client = httpx.AsyncClient(
            timeout=max(1.0, float(settings.data_sync.hot_news_http_timeout_seconds)),
...
class HotNewsCollector(BaseCollector):
    ...
    default_schedule = "*/30 * * * *"  # 保守默认：每30分钟执行一次
    recoverability = {
        "mode": "none",
        "can_rerun": False,
        "severity_on_missing": "warning",
        "reason": "多来源热点榜单和快讯具有强时效性，错过采集窗口后无法可靠还原当时排序与热度。",
    }
    ...
    async def collect(self) -> Dict[str, Any]:
        ...
        并发抓取所有来源，统一存入 Redis
        """
        if not settings.data_sync.hot_news_enabled:
            self.logger.info("Hot news collection skipped: disabled by SYNC_HOT_NEWS_ENABLED")
            return {
                "success": False,
                "skipped": True,
                "reason": "hot_news_disabled",
                "count": 0,
            }
        ...
        # 创建所有来源实例
        source_classes = self._select_source_classes()
        if not source_classes:
            self.logger.warning("Hot news collection skipped: no source selected")
            return {
                "success": False,
                "skipped": True,
                "reason": "hot_news_no_sources",
                "count": 0,
            }

        sources = [cls() for cls in source_classes]

        # 并发抓取。默认限制并发，避免高频新闻任务挤占核心同步资源。
        ...
        total_timeout = max(1.0, float(settings.data_sync.hot_news_total_timeout_seconds))
        try:
            ...
            self.logger.error("Hot news collection timed out after %.1fs", total_timeout)
            for source in sources:
                await source.close()
            return {
                "success": False,
                "error": f"hot_news_total_timeout_after_{total_timeout:g}s",
                "reason": "hot_news_total_timeout",
                "count": 0,
            }
    ...
    @classmethod
    def _parse_source_allowlist(cls) -> set[str]:
        raw_value = settings.data_sync.hot_news_source_allowlist or ""
        return {item.strip() for item in raw_value.split(",") if item.strip()}

    @classmethod
    def _select_source_classes(cls) -> List[type]:
        allowlist = cls._parse_source_allowlist()
        source_classes = [
            source_class
            for source_class in cls.SOURCE_CLASSES
            if not allowlist or source_class.name in allowlist
        ]
        max_sources = max(1, int(settings.data_sync.hot_news_max_sources_per_round))
        return source_classes[:max_sources]
    ...
    async def _fetch_source(self, source: HotNewsSource) -> Dict[str, Any]:
        ...
        try:
            ...
            max_items = max(1, int(settings.data_sync.hot_news_max_items_per_source))
