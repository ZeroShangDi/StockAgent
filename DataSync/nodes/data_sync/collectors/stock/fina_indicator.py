...
class FinaIndicatorCollector(BaseCollector):
    ...
    async def _sync_all_stocks(
        ...
        async def process_stock(ts_code: str) -> int:
            """处理单只股票的财务数据"""
            financial_data, source = await data_source_manager.get_financial_data(
                ts_code=ts_code,
                limit=limit,
            ...
            if source and source != self.PREFERRED_SOURCE:
                self.logger.warning(
                    f"Financial data for {ts_code} fell back to {source}, expected {self.PREFERRED_SOURCE}"
                )
