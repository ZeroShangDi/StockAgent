...
class DailyBasicCollector(BaseCollector):
    ...
    resource_class = "core"
    target_collections = ("daily_basic",)
    dependencies = ("stock_basic", "trade_calendar")
    source_chain_keys = ("get_latest_trade_date", "get_daily_basic.full_market", "get_daily_basic.single")
    supports_backfill = True
    quality_checks = ("required_fields:ts_code,trade_date", "dedupe_key:ts_code,trade_date", "numeric_normalization")
    ...
    async def collect(self) -> Dict[str, Any]:
        ...
        if focus_mode:
            ...
            total_count, result = await self._collect_trade_dates(trade_dates)
    ...
    async def recover_trade_date(self, trade_date: str) -> Dict[str, Any]:
        """定向补指定交易日的 daily_basic，不回退 sync marker。"""
        ...
        return {
            ...
            "failed_dates": result["failed"],
            "failed_items": result.get("failed_items", []),
            "message": f"Recovered daily_basic for {trade_date}",
        }
    ...
    async def _collect_trade_dates(self, trade_dates: List[str]) -> tuple[int, Dict[str, Any]]:
        ...
        async def collect_single_date(trade_date: str) -> Dict[str, Any]:
            ...
            records, source = await data_source_manager.get_daily_basic(trade_date=trade_date)
