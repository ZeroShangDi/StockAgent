...
class MoneyflowConceptCollector(BaseCollector):
    ...
    dataset_name = "moneyflow_concept"
    resource_class = "core"
    target_collections = ("moneyflow_concept",)
    dependencies = ("trade_calendar",)
    source_chain_keys = ("get_latest_trade_date", "get_moneyflow_concept")
    ...
    async def collect(self) -> Dict[str, Any]:
        ...
        if already_synced and not coverage_ok:
            self.logger.warning(
                ...
                "but moneyflow_concept coverage is incomplete; forcing latest-date resync"
            )
    ...
    async def recover_trade_date(self, trade_date: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "failed_dates": result["failed"],
            "failed_items": result.get("failed_items", []),
            "message": f"Recovered moneyflow_concept for {trade_date}",
        }
    ...
    async def _collect_trade_dates(self, trade_dates: List[str]) -> tuple[int, Dict[str, Any]]:
        ...
        async def collect_single_date(trade_date: str) -> Dict[str, Any]:
            nonlocal total_count
            records, source = await data_source_manager.get_moneyflow_concept(trade_date=trade_date)
            if records:
                count = await self._write_buffer(
                    buffer=records,
                    collection="moneyflow_concept",
